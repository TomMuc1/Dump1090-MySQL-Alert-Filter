...
while (true) {
	...
	foreach ($json_data_array['aircraft'] as $row) {
		...
		if ($user_set_array['filter_mode_database']) {
			if (($ac_altitude < $user_set_array['max_alt'] and $ac_lat < $user_set_array['max_lat'] and $ac_lat > $user_set_array['min_lat'] and $ac_lon < $user_set_array['max_lon'] and $ac_lon > $user_set_array['min_lon']) and (in_array($ac_hex, $hex_code_array) or ($ac_flight != '') and (in_array($ac_flight, $flight_code_array)))) {
